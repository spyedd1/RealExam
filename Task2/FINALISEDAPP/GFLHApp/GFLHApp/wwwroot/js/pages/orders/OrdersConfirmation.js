...
(function () { // Starts an isolated script scope for this page.
    ...
    // ----- DOM references -----
    var hero = document.querySelector('.oc-hero'); // Stores the hero DOM element reference.
    var floats = Array.prototype.slice.call(document.querySelectorAll('.oc-hero__float')); // Stores the floats DOM element reference.
    // ----- Helpers -----
    ...
    function draw() { // Defines the draw helper function.
        ...
        floats.forEach(function (el, index) { // Starts a JavaScript block for the current control flow.
            ...
            // ----- State updates -----
            el.style.setProperty('--oc-x', (currentX * depth).toFixed(2) + 'px'); // Updates inline style for a dynamic UI state.
            el.style.setProperty('--oc-y', (currentY * depth).toFixed(2) + 'px'); // Updates inline style for a dynamic UI state.
        });
    ...
    // ----- DOM references -----
    var confirmMark = document.querySelector('.oc-confirm-mark'); // Stores the confirmMark DOM element reference.
    if (confirmMark) { // Checks the condition before running the next script step.
        // ----- Helpers -----
        setTimeout(function () { // Starts a JavaScript block for the current control flow.
            // ----- State updates -----
            confirmMark.classList.add('oc-pop'); // Adds a CSS class to update the element state.
        }, 250); // Runs this JavaScript step for the page interaction.
    }

    // ----- DOM references -----
    document.querySelectorAll('.oc-grower-card').forEach(function (card) { // Finds a page element needed by the script.
        // ----- Event wiring -----
        card.addEventListener('mouseenter', function () { // Registers an event handler for user or browser interaction.
            // ----- State updates -----
            card.style.transform = 'translateY(-2px)'; // Updates inline style for a dynamic UI state.
            card.style.boxShadow = 'var(--shadow-md)'; // Updates inline style for a dynamic UI state.
        });
