...
namespace GFLHApp.Controllers // Places these MVC controller types in the application controllers namespace.
{
    ...
    public class OrdersController : Controller // Defines the MVC controller for checkout, order history, delivery confirmation, and reordering.
    {
        ...
        public async Task<IActionResult> Create(int basketId) // Shows or processes the create form for a record
        {
            ...
            foreach (var item in basketProducts) // Iterates through each matching item to process it.
            {
                subtotal += item.Products.ItemPrice * item.ProductQuantity; // Sets + to the value needed by the workflow.
            }
            ...
            // Basket totals
            decimal discountedSubtotal = subtotal - discount; // Calculates the discountedSubtotal value used for totals.

            // Basket lookup
            ViewBag.BasketId = basketId; // Supplies BasketId to the view for display or form state.
            ViewBag.Subtotal = discountedSubtotal; // Supplies Subtotal to the view for display or form state.
            // Loyalty system
            ...
            ViewBag.DeliveryCosts = new Dictionary<string, decimal> // Supplies DeliveryCosts to the view for display or form state.
    {
        ...
        public async Task<IActionResult> Create([Bind("OrdersId,Delivery,Collection,DeliveryMethod,DateOfCollection,BillingLine1,BillingLine2,BillingCity,BillingPostcode,DeliveryLine1,DeliveryLine2,DeliveryCity,DeliveryPostcode,TermsAccepted")] Orders orders, int basketId) // Shows or processes the create form for a record
        {
            ...
            // Basket totals
            decimal subtotal = 0.00m; // Calculates the subtotal value used for totals.
            foreach (var basketProduct in basketProducts) // Iterates through each matching item to process it.
            {
                ...
                subtotal = productTotal + subtotal; // Calculates the subtotal value used for totals.
            }

            // Order logic
            var orderCouut = await _context.Orders.CountAsync(x => x.UserId == userId); // Counts matching records for display or validation.

            var basketProductNames = basketProducts.Select(x => x.Products.ItemName.ToLower()).ToList(); // Projects the query results into the shape needed next.
            // Health bundle
            bool hasHealthBundle = basketProductNames.Contains("broccoli") && // Starts the health bundle check by looking for broccoli.
                                   basketProductNames.Contains("carrot") && // Continues the health bundle check by requiring carrot.
                                   basketProductNames.Contains("apple"); // Completes the health bundle check by requiring apple.

            // Delivery logic
            decimal deliveryCost = 0m; // Sets deliveryCost to the value needed by the workflow.
            if (orders.Delivery) // Checks the condition that controls the next action.
            {
                if (orderCouut % 3 == 2) // Checks the condition that controls the next action.
                {
                    deliveryCost = 0m; // Sets deliveryCost to the value needed by the workflow.
                }
                else // Handles the fallback branch when earlier conditions did not match.
                {
                    // Delivery logic
                    deliveryCost = orders.DeliveryMethod switch // Sets deliveryCost to the value needed by the workflow.
                    {
                        "Next Day" => 5.99m, // Sets the next-day delivery charge.
                        "Standard" => 2.99m, // Sets the standard delivery charge.
                        "Economy" => 0.99m, // Sets the economy delivery charge.
                        _ => 0m // Falls back to no delivery charge for unknown methods.
                    };
                }
            }

            decimal discount = 0m; // Calculates the discount value used for totals.
            if (orderCouut % 5 == 4) // Checks the condition that controls the next action.
            {
                discount = subtotal * 0.15m; // Calculates the discount value used for totals.
            }
            // Health bundle
            ...
            // Basket totals
            orders.OrdersTotal = (subtotal - discount) + deliveryCost; // Calculates the orders.OrdersTotal value used for totals.
