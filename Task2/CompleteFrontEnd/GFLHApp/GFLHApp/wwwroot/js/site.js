...
/* ── 0. Smooth anchor scroll (explicit, cross-browser) ───── */
(function () {
...
/* ── 2. Scroll reveal (ScrollReveal.js) ──────────────────── */
...
/* ── 3. Hero parallax (RAF lerp, passive) ────────────────── */
(function () {
    var hero = document.querySelector('.pw-hero');
    var floats = Array.prototype.slice.call(
        document.querySelectorAll('.pw-hero__float'));
    if (!hero || !floats.length) return;

    var depths = [14, 20, 9, 16, 7, 22];
    ...
    function step() {
        ...
        raf = (Math.abs(cX - tX) < 0.002 && Math.abs(cY - tY) < 0.002)
            ? null
...
(function () {
    ...
    /* Close menu on nav link click (single-page navigation) */
    drawer.querySelectorAll('a').forEach(function (a) {
        a.addEventListener('click', closeMenu);
    });
}());

/* ── 7. Stat counter (ease-out cubic, fires once) ─────────── */
(function () {
    var els = document.querySelectorAll('.pw-stat__value[data-target]');
