...
namespace GFLHApp.Data // Places this class in the application data namespace.
{
    ...
    public class ApplicationDbContext : IdentityDbContext // Defines the EF Core database context for the application.
    {
        ...
        public DbSet<ContactInquiry> ContactInquiries { get; set; } = default!; // Exposes the contact inquiry table to EF Core queries.
        ...
        protected override void OnModelCreating(ModelBuilder builder) // Configures EF Core model rules and relationships.
        {
            ...
            builder.Entity<ContactInquiry>() // Keeps the contact inquiry schema stable while frontend validation lives in the UI.
            ...
            builder.Entity<ContactInquiry>() // Stores the admin reply as long-form text attached to the inquiry.
                .Property(i => i.AdminReply)
                .HasMaxLength(3000);

            builder.Entity<ContactInquiry>() // Stores which admin account wrote the reply for traceability.
                .Property(i => i.RepliedByEmail)
                .HasMaxLength(256);
        }
        // ----- DbSet properties -----
