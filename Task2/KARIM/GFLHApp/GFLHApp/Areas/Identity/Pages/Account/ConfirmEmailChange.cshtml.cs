...
namespace GFLHApp.Areas.Identity.Pages.Account // Places this page model in the Identity area namespace.
{
    ...
    public class ConfirmEmailChangeModel : PageModel // Defines the Razor Page model class for this page.
    {
        // ----- Injected Services -----
        ...
        public ConfirmEmailChangeModel(UserManager<IdentityUser> userManager, SignInManager<IdentityUser> signInManager) // Receives services from dependency injection.
        {
        ...
        // ----- Page Handlers -----
        public async Task<IActionResult> OnGetAsync(string userId, string email, string code) // Handles GET requests that display this page.
        {
            // ----- Email Logic -----
            if (userId == null || email == null || code == null) // Checks the condition before continuing this page flow.
            {
            ...
            // ----- Injected Services -----
            var result = await _userManager.ChangeEmailAsync(user, email, code); // Changes the user's email address after confirmation.
            if (!result.Succeeded) // Checks the condition before continuing this page flow.
            {
                // ----- Email Logic -----
                StatusMessage = "Error changing email."; // Stores a status message for the Razor Page to show.
                // ----- Redirects and Results -----
            ...
            // ----- Injected Services -----
            var setUserNameResult = await _userManager.SetUserNameAsync(user, email); // Runs the Identity operation asynchronously.
            if (!setUserNameResult.Succeeded) // Checks the condition before continuing this page flow.
            {
                StatusMessage = "Error changing user name."; // Stores a status message for the Razor Page to show.
                // ----- Redirects and Results -----
                return Page(); // Renders the current Razor Page.
            }
            ...
            // ----- Email Logic -----
            StatusMessage = "Thank you for confirming your email change."; // Stores a status message for the Razor Page to show.
            // ----- Redirects and Results -----
            return Page(); // Renders the current Razor Page.
        }
    }
