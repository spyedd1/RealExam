...
namespace GFLHApp.Controllers // Places these MVC controller types in the application controllers namespace.
{
    ...
    public class ProducersController : Controller // Defines the MVC controller for producer profiles, account creation, VAT details, and producer images.
    {
        ...
        // Role management
        [Authorize(Roles = "Developer,Admin,Producer")] // Restricts access to users in the Developer,Admin,Producer role set.
        ...
        public async Task<IActionResult> Edit(int? id) // Shows or processes edits for an existing record
        {
            ...
            if (producers == null) return NotFound(); // Checks whether the requested data was found.
            if (!CanManageProducer(producers)) return Forbid(); // Allows producers to edit only their own profile.
            ...
            string? existingImagePath) // Receives the previously selected image path when no new file is uploaded.
        {
            ...
            if (!CanManageProducer(existingProducer)) return Forbid(); // Allows producers to update only their own profile.
            ...
            {
                // Edit record
                if (User.IsInRole("Admin") || User.IsInRole("Developer")) // Allows privileged users to repair producer account links.
                {
        ...
        private bool CanManageProducer(Producers producers) // Checks whether the signed-in user may edit this producer profile
        {
            if (User.IsInRole("Admin") || User.IsInRole("Developer")) // Allows privileged staff to manage any producer.
            {
                return true; // Returns true because this role has broad producer management access.
            }
            ...
            return User.IsInRole("Producer")
                   ...
                   && producers.UserId == userId; // Allows producers to manage only their own profile.
        }
